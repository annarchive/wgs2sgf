...
long long wgsFile::getrank(char* filepath,int p)
{
	...
	if(rank<0)
		r=r-2;
	if(rank>=0&&rank<100)	//考虑0~100之间为1级的情况 如40/100=0,0-1 = -1为1级
		r=r-1;
...
{
	brank=getrank(filepath,36);		//黑棋段位,36字节处
	wrank=getrank(filepath,84);		//白棋段位,84字节处
	blackdan=getdan(filepath,42);	//黑棋评测段位,42字节处
	whitedan=getdan(filepath,90);	//白棋评测段位,90字节处
	read_wgs.open(filepath,ios::in|ios::binary);
	isfileopen(filepath);
	string str="BR[";
	string tmpstr="";
...
string wgsFile::getresult(char* filepath)
{
	...
	//点目结果，108,109字节，倒序取补码，除2再减0.25,为正黑胜，为负黑负
	 ...
	 *下面这段代码有点问题，胜负并不取决于第106字节，但是正常情况下(非人为改动wgs文件)是可以标识胜负的。qq围棋认的是点目结果，正数黑胜，负数黑败。
	 *上面一句已经被推翻。中盘结束时胜负确实依赖106字节，所以不如统一以106字节处理，人为改动的就不考虑支持了.*/
	...
	stringstream stream;
	stream<<abs(point);		//显示的时候当然不能显示负号了
	stream>>pts;
	stream.clear();
	stream.str("");
